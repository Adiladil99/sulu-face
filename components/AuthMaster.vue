...
const onSubmit = handleSubmit(async values => {
  const result = await useFetchBack("auth-master/register", {
    body: values,
    method: "post"
  })
});
...
const onLoginSubmit = async values => {
  await authenticateUser(values, 'master'); // call authenticateUser and pass the user object
  // redirect to homepage if user is authenticated
  if (authenticated) {
    router.push('/profile');
    emit("close");
  }
  // const result = await useFetchBack("auth-master/login", {
  //   body: values,
  //   method: "post"
  // })
  // console.log(result);
      ...
      <div class="modal-content">
        ...
        <div class="form-container" v-else>
          ...
          <Form class="form" @submit="onSubmit">
            ...
            <input type="text" class="input" placeholder="Адрес" v-model="address" v-bind="addressAttrs" />
            <input type="email" class="input" placeholder="Email" v-model="email" v-bind="emailAttrs" />
